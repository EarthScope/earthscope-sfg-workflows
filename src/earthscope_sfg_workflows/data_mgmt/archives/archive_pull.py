...
def retrieve_token(profile=None):
    ...
    try:
        ...
        try:
            ...
            raise ArchiveAuthError(f"Failed to refresh token: {exc}") from exc

    token = es.ctx.auth_flow.access_token
    if not token:
        raise ArchiveAuthError("Failed to retrieve access token after authentication.")
    return token
...
def load_vessel_metadata(
    ...
    if local_path is not None:
        ...
        logger.info(f"Loading vessel metadata from {vessel_json_url}")
...
def load_site_metadata(network: str, station: str, profile: str = None) -> Site:
    ...
    for campaign in site.campaigns:
        try:
            ...
            logger.error(f"Vessel metadata file not found for campaign {campaign.name}: {e}")
            campaign.vessel = None
        except ValueError as e:
            logger.error(f"Invalid vessel metadata for campaign {campaign.name}: {e}")
            campaign.vessel = None
...
def list_campaign_files(network: str, station: str, campaign: str) -> list:
    ...
    logger.info(f"Listing raw campaign files from url {raw_url}")

    raw_file_list = list_files_from_archive(raw_url)
    list_file_counts_by_type(file_list=raw_file_list, url=raw_url)

    logger.info(f"Listing metadata campaign files from url {metadata_url}")
    ...
    logger.info(f"Listing RINEX 1Hz campaign files from url {rinex_1hz_url}")
    ...
    logger.info(f"Listing RINEX 10Hz campaign files from url {rinex_10hz_url}")
